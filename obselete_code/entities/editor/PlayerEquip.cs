...
[HammerEntity]
[SandboxEditor.EditorSprite( "materials/dev/editor/dm04playerequip.vmat" )]
...
public partial class PlayerSetup : Entity
{
	...
	public override void Spawn()
	{
		...
		if ( Should_Give_Crowbar )
			weaponsToGive.Add( TypeLibrary.Create<BaseDmWeapon>( "hl2_crowbar" ) );

		if ( Should_Give_USP )
			weaponsToGive.Add( TypeLibrary.Create<BaseDmWeapon>( "hl2_uspmatch" ) );

		if ( Should_Give_Revolver )
			weaponsToGive.Add( TypeLibrary.Create<BaseDmWeapon>( "hl2_357" ) );

		if ( Should_Give_SMG )
			weaponsToGive.Add( TypeLibrary.Create<BaseDmWeapon>( "hl2_smg1" ) );

		if ( Should_Give_Shotgun )
			weaponsToGive.Add( TypeLibrary.Create<BaseDmWeapon>( "hl2_spas12" ) );

		if ( Should_Give_AR2 )
			weaponsToGive.Add( TypeLibrary.Create<BaseDmWeapon>( "hl2_ar2" ) );

		if ( Should_Give_Bow )
			weaponsToGive.Add( TypeLibrary.Create<BaseDmWeapon>( "hl2_crossbow" ) );

		if ( Should_Give_Grenade )
			weaponsToGive.Add( TypeLibrary.Create<BaseDmWeapon>( "hl2_grenade" ) );

		if ( Should_Give_RPG )
			weaponsToGive.Add( TypeLibrary.Create<BaseDmWeapon>( "hl2_rpg" ) );

		if ( Should_Give_Slam )
			weaponsToGive.Add( TypeLibrary.Create<BaseDmWeapon>( "hl2_slam" ) );

		//Anything below here is wip weapons.

		if ( Should_Give_Bugbait )
			weaponsToGive.Add( TypeLibrary.Create<BaseDmWeapon>( "hl2_bugbait" ) );
	}
			...
			if (client.Pawn is DeathmatchPlayer player && player.DidRespawn )
			{
				foreach ( var weapon in weaponsToGive )
				{
					if( !player.Inventory.Contains(weapon) )
						player.Inventory.Add( TypeLibrary.Create<BaseDmWeapon>(weapon.GetType()) );
