...
[Hammer.EntityTool( "Player Equip", "DM:04", "Manipulates the Player Arsenal" )]
...
public partial class PlayerSetup : Entity
{
	...
	public override void Spawn()
	{
		...
		if ( Should_Give_Shotgun )
			weaponsToGive.Add( Library.Create<BaseDmWeapon>( "hl2_spas12" ) );

		if ( Should_Give_AR2 )
			weaponsToGive.Add( Library.Create<BaseDmWeapon>( "hl2_ar2" ) );

		if ( Should_Give_Bow )
			weaponsToGive.Add( Library.Create<BaseDmWeapon>( "hl2_crossbow" ) );

		if ( Should_Give_Grenade )
			weaponsToGive.Add( Library.Create<BaseDmWeapon>( "hl2_grenade" ) );

		if ( Should_Give_RPG )
			weaponsToGive.Add( Library.Create<BaseDmWeapon>( "hl2_rpg" ) );
